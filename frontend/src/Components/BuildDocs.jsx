...
import {
  AlignLeft, Copy, ExternalLink, Eye, FileChartPie,
  Play, UserPlus, Plus, FileText, Dot, Download, Link
} from 'lucide-react';
...
function BuildDocs() {
  ...
  const iconMap = {
    Dot, AlignLeft, Copy, ExternalLink, FileChartPie,
    Play, UserPlus, Plus, FileText
  };
  ...
  if (!projectData) return <SkProjectCardFour />;
  ...
  return (
    <div className="h-auto min-h-screen w-full p-2 md:p-4">
      <div className="flex flex-col md:flex-row h-full gap-3">
        ...
        {/* Attachment Panel */}
        <div className="w-full md:w-1/5 h-fit md:h-full border-2 border-[#b3b9c4] bg-white">
          <li className="flex items-center justify-center w-full h-10 pl-3 pr-2 bg-black text-white font-medium">
            <span className="text-md">Attachments</span>
          </li>

          <ul className="list-none pl-4 pr-2 flex flex-col gap-2 mt-4">
            <li onClick={() => settextDocModel(true)}
        ...
        <div className="w-full md:w-4/5 flex flex-col items-center bg-[#f3f4f6]">
          ...
          <div className="border-[1px] bg-white border-gray-500 w-full max-w-[750px] min-h-[60vh] pb-[6px] overflow-hidden">
            {/* Header */}
                      ...
                      {files.map((file, index) => (
                        <div key={index}
                          ...
                          <div className="flex flex-col justify-start">
                            <h6 className="font-semibold text-gray-800 text-[7px]">{file.fileText.slice(0, 11)}..</h6>
          ...
          <p
            className='text-md text-blue-400 hover:text-blue-800 my-1 cursor-pointer'
            onClick={() => navigate(`/documentation/${projectName}`)}>
            Click to view All ...
          </p>

          {textDocModel && <TextDocumentModel onClose={() => settextDocModel(false)} />}
          {ImageModel && <ImageDocument onClose={() => setImageDocModel(false)} projectName={projectName} />}
          {FileModel && <FileDocument onClose={() => setFileDocModel(false)} projectName={projectName} />}
        </div>
      </div>
    </div>
  );
